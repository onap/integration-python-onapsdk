...
def test_sdnc_service_gr_api_get(mock_send_message_json):
    mock_send_message_json.return_value = SDNC_SERVICE_INFORMATION
    service = Service(SDNC_SERVICES_INFORMATION["services"]["service"][0]["service-instance-id"],
                      SDNC_SERVICES_INFORMATION["services"]["service"][0]["service-status"])
    service_get = service.get()
    assert isinstance(service_get, Iterable)
    service_instance = next(service_get)
    assert isinstance(service_instance, Service)
    assert service_instance.service_instance_id == SDNC_SERVICE_ID
