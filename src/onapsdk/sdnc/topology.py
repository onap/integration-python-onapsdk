...
class Topology(SdncElement):
    ...
    def get_all(cls) -> Iterable["Topology"]:
        ...
        topologies = cls.send_message_json("GET",
                                           ...
                                           ).get("network-topology:network-topology", {}).get("topology", [])
        for topology in topologies:
            try:
                yield Topology(topology_id=topology["topology-id"], nodes=topology["node"])
            except KeyError:
                print(f"Topology with topology-id={topology['topology-id']} doesn't contain any node")
    ...
    def get(cls, topology_id) -> "Topology":
        ...
        try:
            ...
            return Topology(topology_id=topology["topology-id"],
                ...
                print(f"Topology with topology-id={topology_id} doesn't contain any node")
                yield Topology(topology_id=topology_id)
    ...
    def get_node(self, node_id) -> "Node":
        ...
        try:
            ...
            print(f"Error. Node creation skipped: {e}")
